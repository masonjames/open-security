...
MAX_RECOVERY_COLLECTION_ITEMS = 4096
MAX_RECOVERY_WARNINGS = 4096
MAX_RECOVERY_WARNING_CHARS = 1000
...
def _bounded_recovery_warning(warning: str) -> str:
    if len(warning) <= MAX_RECOVERY_WARNING_CHARS:
        return warning
    return f"{warning[: MAX_RECOVERY_WARNING_CHARS - 3]}..."


def _append_recovery_warning(warnings: list[str], warning: str) -> None:
    warning = _bounded_recovery_warning(warning)
    if len(warnings) < MAX_RECOVERY_WARNINGS - 1:
        warnings.append(warning)
    elif len(warnings) == MAX_RECOVERY_WARNINGS - 1:
        warnings.append(
            "Additional artifact-recovery warnings were omitted after the bounded limit."
        )


def _require_recovery_collection_bound(items: list[Any], context: str) -> None:
    if len(items) > MAX_RECOVERY_COLLECTION_ITEMS:
        raise ContractError(
            f"{context}: recovery supports at most {MAX_RECOVERY_COLLECTION_ITEMS} items"
        )
...
def _recover_unsealed_findings(
    ...
    draft_findings = _require_list(findings, "findings", "findings")
    _require_recovery_collection_bound(draft_findings, "findings.findings")
    for index, finding in enumerate(draft_findings):
        context = f"findings.findings[{index}]"
        try:
            if not isinstance(finding, dict):
                raise ContractError(f"{context}: expected an object")
            ...
            if "writeup" in finding:
                try:
                    ...
                    _append_recovery_warning(
                        warnings,
                        f"Skipped malformed writeup for finding {index + 1}: {exc}.",
                    )
            _validate_schema_node(finding, finding_schema, context)
        except ContractError as exc:
            warning = _bounded_recovery_warning(
                f"Skipped malformed finding {index + 1}: {exc}."
            )
            _append_recovery_warning(warnings, warning)
...
def _recover_unsealed_coverage(
    ...
    for field, label in (
        ...
        if not isinstance(items, list):
            _append_recovery_warning(
                warnings, f"Skipped malformed {label} records: expected an array."
            )
            coverage[field] = []
            partial = True
            continue
        _require_recovery_collection_bound(items, f"coverage.{field}")
        if field == "surfaces":
            receipt_ref_count = sum(
                len(item.get("receiptRefs", []))
                for item in items
                if isinstance(item, dict) and isinstance(item.get("receiptRefs"), list)
            )
            if receipt_ref_count > MAX_RECOVERY_COLLECTION_ITEMS:
                raise ContractError(
                    "coverage.surfaces receiptRefs: recovery supports at most "
                    f"{MAX_RECOVERY_COLLECTION_ITEMS} items"
                )
        ...
        for index, item in enumerate(items):
            ...
            try:
                ...
                _append_recovery_warning(
                    warnings, f"Skipped malformed {label} {index + 1}: {exc}."
                )
                partial = True
                continue
...
def _recover_unsealed_hardening(
    ...
    try:
        ...
        scan.pop("hardening")
        _append_recovery_warning(
            warnings, f"Skipped malformed hardening portfolio: {exc}."
        )
