...
def normalize_pre_release_execution_profile_migrations(
    ...
    execution_migrations = {
        row["version"]: row["name"]
        for row in connection.execute(
            "SELECT version, name FROM schema_migrations WHERE version IN (11, 12, 25, 26, 27)"
        )
    }
    legacy_names = {
    ...
    if has_public_warnings_history:
        ...
        if migration_27 not in (None, prepared_completion_migration_name):
            raise SystemExit(
                "The Codex Security database has an unsupported pre-release migration history."
            )
        if migration_26 is not None:
            if (
                ...
                or execution_migrations.get(27) is not None
            ):
                raise SystemExit(
                    "The Codex Security database has an unsupported pre-release migration history."
                )
            connection.execute(
                "UPDATE schema_migrations SET version = 27 WHERE version = 26 AND name = ?",
                (prepared_completion_migration_name,),
            )
            execution_migrations[27] = prepared_completion_migration_name
