...
def normalize_pre_release_execution_profile_migrations(
    ...
    execution_migrations = {
        ...
        for row in connection.execute(
            "SELECT version, name FROM schema_migrations WHERE version IN (11, 12, 25, 26, 27, 28)"
        )
    }
    legacy_names = {
    ...
    prepared_completion_migration_name = "bind prepared scan completions"
    deep_failure_migration_name = "persist deep scan consecutive discovery failures"
    ...
    if execution_migrations.get(27) == prepared_completion_migration_name:
        if execution_migrations.get(28) is not None:
            raise SystemExit(
                "The Codex Security database has an unsupported pre-release migration history."
            )
        connection.execute(
            "UPDATE schema_migrations SET version = 28 WHERE version = 27 AND name = ?",
            (prepared_completion_migration_name,),
        )
        execution_migrations[28] = prepared_completion_migration_name
        execution_migrations.pop(27)
    ...
    if has_public_warnings_history:
        migration_26 = execution_migrations.get(26)
        migration_27 = execution_migrations.get(27)
        migration_28 = execution_migrations.get(28)
        if migration_27 not in (None, deep_failure_migration_name):
            raise SystemExit(
                "The Codex Security database has an unsupported pre-release migration history."
            )
        if migration_28 not in (None, prepared_completion_migration_name):
            raise SystemExit(
                "The Codex Security database has an unsupported pre-release migration history."
            )
        if migration_26 is not None:
            if (
                migration_26 != prepared_completion_migration_name
                or execution_migrations.get(28) is not None
            ):
                raise SystemExit(
                    "The Codex Security database has an unsupported pre-release migration history."
                )
            connection.execute(
                "UPDATE schema_migrations SET version = 28 WHERE version = 26 AND name = ?",
                (prepared_completion_migration_name,),
            )
            execution_migrations[28] = prepared_completion_migration_name
            execution_migrations.pop(26)
        connection.execute(
...
def normalize_pre_release_migrations(connection: sqlite3.Connection, timestamp: str) -> None:
    ...
    if (
        ...
        migration_26 = connection.execute(
            "SELECT name FROM schema_migrations WHERE version = 26"
        ).fetchone()
        if migration_26 is not None:
            migration_28 = connection.execute(
                "SELECT name FROM schema_migrations WHERE version = 28"
            ).fetchone()
            if (
                migration_26["name"] != "bind prepared scan completions"
                or migration_28 is not None
            ):
                raise SystemExit(
                    "The Codex Security database has an unsupported pre-release migration history."
                )
