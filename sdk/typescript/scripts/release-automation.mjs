...
const forkPackageName = "@masonjames/open-security";
const releasePackageName = process.env["CODEX_SECURITY_RELEASE_PACKAGE_NAME"];
const acceptedPackageNames = new Set([packageName, forkPackageName]);
...
export function releaseVersion(packageJson) {
  const accepted =
    releasePackageName === undefined
      ? acceptedPackageNames.has(packageJson?.name)
      : packageJson?.name === releasePackageName;
  if (!accepted) {
    throw new Error(
      releasePackageName === undefined
        ? "Release package must be @openai/codex-security or @masonjames/open-security."
        : `Release package must be ${releasePackageName}.`,
    );
  }
  if (
