...
function printScanSummary(
  ...
  errorOutput.write(`open-security: Elapsed: ${elapsed}s.\n`);
  ...
  const reportPath = cliErrorMessage(result.reportPath);
  const scanDir = cliErrorMessage(result.scanDir);
  errorOutput.write(`open-security: Report: ${reportPath}\n`);
  ...
  errorOutput.write(
    result.sarifPath === null
      ...
      : `open-security: Next: review ${reportPath}\n`,
  );
}
