...
export async function main(
  ...
  const history = async (
    ...
    try {
      ...
      errorOutput.write(`open-security: ${redactedErrorMessage(error)}\n`);
      exitCode = 2;
      return undefined;
    }
  };
    ...
    history(
      ...
      async ({ matchingCached, matchingInputs, ...comparison }) => {
        ...
        const comparisonInput = matchingInputs as JsonObject &
          ScanComparisonInput;
        const requiresModel =
          comparisonInput.before.length > 0 && comparisonInput.after.length > 0;
        if (requiresModel) {
          rejectUnsupportedScanCostLimit(
  ...
  if (frameworkExit !== undefined) {
    ...
    errorOutput.write(
      `open-security: ${redactedErrorMessage(incurErrorMessage(frameworkOutput))}\n`,
    );
    return 2;
  }
...
function interruptedExit(
  ...
  errorOutput.write(
    scanDir === null
      ...
      : `open-security: Partial output was kept at ${redactedErrorMessage(scanDir)}.\n`,
  );
...
if (invokedAsMain()) {
  void main().then(
    ...
    (error: unknown) => {
      process.stderr.write(`open-security: ${redactedErrorMessage(error)}\n`);
      process.exitCode = 2;
    },
  );
}
