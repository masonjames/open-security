...
export async function mergedCodexConfig(
  ...
  if (selection.provider !== "openai" && selection.model === undefined) {
    throw new ConfigurationError(
      `${selection.provider} scans require an explicit model via --model, codexOverrides.model, or OPEN_SECURITY_MODEL.`,
    );
  }
