...
export function requireArchiveSafeParentDirectory(
  ...
  if (writableByOtherUsers && !trustedStickyAncestor) {
    throw new OutputDirectoryError(
      `Private scan output requires a parent directory that other users cannot rewrite; only an explicitly trusted sticky parent is allowed for shared locations: ${path}`,
    );
  }
}
...
export function requirePrivateOutputDirectory(
  ...
  if ((metadata.mode & 0o777) !== 0o700) {
    throw new OutputDirectoryError(
      `Scan output directory must not be accessible to other users and must use owner-only read, write, and execute permissions (chmod 700): ${path}`,
    );
  }
  if (effectiveUid === undefined || metadata.uid !== effectiveUid) {
    throw new OutputDirectoryError(
...
export function requireTrustedOutputAncestor(
  ...
  if ((metadata.mode & 0o1000) === 0) {
    throw new OutputDirectoryError(
      `Scan output requires a parent directory that other users cannot rewrite; a group- or world-writable parent must have the sticky bit: ${path}`,
    );
  }
}
