...
function emptyFalsePositiveFeedback(): JsonObject {
  return {
  ...
  const kind =
    recipe.target.kind === "refs" || recipe.target.kind === "working_tree"
      ...
      : recipe.repositoryRevision === undefined
        ...
        : "git_revision";
  const diffTarget =
    recipe.target.kind === "refs"
      ? {
          kind: "range",
          baseRevision: recipe.target.base,
          headRevision: recipe.target.head,
        }
      : recipe.target.kind === "working_tree"
        ? {
            kind: "working_tree",
            baseRevision: recipe.target.base,
            headRevision: recipe.target.head,
            contentDigest: TEST_SNAPSHOT_DIGEST,
          }
        : null;
...
describe("CodexSecurity orchestration", () => {
  ...
  test("rebuilds a cached runtime when the environment-selected provider changes", async () => {
    const root = await temporaryDirectory();
    const repository = join(root, "repository");
    await mkdir(repository);
    ...
    const client = new TestClient(
      {
        codexOverrides: {
          model: "synthetic/provider-change",
      ...
      },
      {
        ...
        fetchOpenRouterModel: async (model: string) => ({
          id: model,
          canonicalSlug: null,
          name: "Synthetic provider-change model",
        ...
        prepareRuntime: async () => {
          preparationCount += 1;
          const codexHome = join(root, `codex-home-${preparationCount}`);
          await mkdir(codexHome);
          return {
            ...preparedRuntime(codexHome),
            ...(environment["OPEN_SECURITY_PROVIDER"] === "openrouter"
              ? {
                  openRouterBridge: {
                    ...syntheticOpenRouterBridge(),
                    async close() {
                      bridgeCloseCount += 1;
                    },
                  },
    ...
    try {
      ...
      environment["OPEN_SECURITY_PROVIDER"] = "openai";
      ...
      await expect(stat(join(root, "codex-home-1"))).rejects.toThrow();
    } finally {
      await client.close();
    }
  });
  ...
  test.each(EXTERNAL_PROVIDER_CASES)(
    ...
    async (name, provider, apiKey, model) => {
      const root = await temporaryDirectory();
      const repository = join(root, "repository");
      await mkdir(repository);
      let runtimeStarted = false;
      ...
      await expect(client.run(repository)).rejects.toThrow(
        `${name} authentication requires ${apiKey}.`,
      );
      expect(runtimeStarted).toBe(false);
      await client.close();
    },
  ...
  test.each(EXTERNAL_PROVIDER_CASES)(
    ...
    async (_name, provider, apiKey, model) => {
      const root = await temporaryDirectory();
      const repository = join(root, "repository");
      const codexHome = join(root, "codex-home");
      const scanDir = join(root, "scan");
      ...
      const client = new TestClient(
        ...
        },
        {
          ...
          ...(provider === "openrouter"
            ? {
                fetchOpenRouterModel: async (requestedModel: string) => ({
                  id: requestedModel,
                  canonicalSlug: null,
                  name: "Synthetic external-provider model",
                  contextLength: 128_000,
                  supportedParameters: [
                    "tools",
                    "response_format",
      ...
      const client = new TestClient(
        {
          ...
          codexOverrides: { model: "openai.gpt-5.6-luna" },
        },
        {
          environment,
          prepareRuntime: async () => ({
      ...
      const client = new TestClient(
        {},
        {
          ...
          runWorkbench: async (_options: unknown, args: readonly string[]) => {
            ...
            registration = args;
            const archiveDir = `${output}.previous-20260729T000000Z-scan_exa`;
  ...
  testArchive(
    "rejects a linked archive returned by scan registration",
    async () => {
      const root = await temporaryDirectory();
      const repository = join(root, "repository");
      const codexHome = join(root, "codex-home");
      const output = join(root, "scan");
      const realArchive = join(root, "real-archive");
      const linkedArchive = `${output}.previous-20260729T000000Z-scan_exa`;
      await mkdir(repository);
      await mkdir(codexHome);
      await mkdir(output, { mode: 0o700 });
      await mkdir(realArchive, { mode: 0o700 });
      await writeFile(join(output, "previous.txt"), "previous scan\n");
      await symlink(realArchive, linkedArchive, "dir");
      ...
      ).rejects.toThrow("invalid scan registration");
      expect(archiveObserved).toBe(false);
      await client.close();
    },
  );
  ...
  test("rejects a working-tree diff registration without its content digest", async () => {
    const root = await temporaryDirectory();
    const repository = join(root, "repository");
    const codexHome = join(root, "codex-home");
    const scanDir = join(root, "scan");
    ...
    await expect(
      client.run(repository, { target: DiffTarget.workingTree() }),
    ).rejects.toThrow("invalid scan registration");
    await client.close();
  });
  ...
  test("routes a real OpenRouter runtime through one private bridge and closes it", async () => {
    ...
    const client = new TestClient(
      ...
      },
      {
        ...
        runWorkbench: async (_options: unknown, args: readonly string[]) => {
          ...
          if (args[0] === "get-scan-feedback") {
            return emptyFalsePositiveFeedback();
          }
          return {};
        },
        createCodex: (options: CodexOptions) => ({
    ...
    const getUpstreamApiKey = bridgeOptions?.["getUpstreamApiKey"] as
      | (() => string)
      | undefined;
    expect(getUpstreamApiKey?.()).toBe("synthetic-openrouter-key");
    environment["OPENROUTER_API_KEY"] = "rotated-openrouter-key";
    expect(getUpstreamApiKey?.()).toBe("rotated-openrouter-key");
    delete environment["OPENROUTER_API_KEY"];
    expect(() => getUpstreamApiKey?.()).toThrow(
...
`,
  ...
  test("preserves a rejected OpenRouter artifact recovery turn", async () => {
    ...
    expect(workbenchCommands[2]).toContain(
...
`,
  ...
  test("prefers managed keyring authentication before importing ambient credentials", async () => {
    const root = await temporaryDirectory();
    const ambientHome = join(root, "ambient-home");
    ...
    expect(probedHome).toBe(credentialHome);
    expect(imported).toBe(false);
