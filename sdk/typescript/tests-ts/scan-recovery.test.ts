...
const describeScans = process.platform === "win32" ? describe.skip : describe;
...
describeScans("malformed scan artifact recovery", () => {
  ...
  test("rejects completion that was not durably prepared", async () => {
    const fixture = await startDraftScan();

    await expect(
      workbench(fixture, ["complete-scan", "--scan-id", fixture.scanId]),
    ).rejects.toThrow(
      "Scan completion must be prepared before it can be published.",
    );
  });

  test("rejects a prepared completion after its manifest changes", async () => {
    const fixture = await startDraftScan();
    await workbench(fixture, [
      "prepare-scan-completion",
      "--scan-id",
    ...
    manifest["unexpectedMutation"] = true;
    await writeJson(manifestPath, manifest);

    await expect(
      workbench(fixture, ["complete-scan", "--scan-id", fixture.scanId]),
    ).rejects.toThrow("The prepared scan manifest changed before completion.");
  });

  test("rejects a prepared completion after a sealed artifact changes", async () => {
    const fixture = await startDraftScan();
    await workbench(fixture, [
      "prepare-scan-completion",
      "--scan-id",
  ...
  test("seals a prepared scan without publishing it before acceptance", async () => {
    ...
    expect((await completeScan(fixture, false)).progress.status).toBe(
      "complete",
    );
  });
  ...
  test("fails closed before recovering excessive finding collections", async () => {
    const fixture = await startDraftScan();
    const path = join(fixture.scanDir, "findings.json");
    const document = await readJson<FindingsDocument>(path);
    document.findings = Array.from({ length: 4097 }, () => null);
    await writeJson(path, document);
    const original = await readFile(path, "utf8");

    await expect(completeScan(fixture)).rejects.toThrow(
      "findings.findings: recovery supports at most 4096 items",
    );
    expect(await readFile(path, "utf8")).toBe(original);
  });

  test("fails closed before recovering excessive receipt references", async () => {
    const fixture = await startDraftScan();
    const path = join(fixture.scanDir, "coverage.json");
    const document = await readJson<CoverageDocument>(path);
    document.surfaces = [
    ...
    await expect(completeScan(fixture)).rejects.toThrow(
      "coverage.surfaces receiptRefs: recovery supports at most 4096 items",
    );
    expect(await readFile(path, "utf8")).toBe(original);
  });
