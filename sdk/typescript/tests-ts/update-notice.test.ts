...
describe("CLI update notice", () => {
  test("checks the npm latest tag and reports a newer version", async () => {
    ...
    expect(requestedUrl).toBe(
      "https://registry.npmjs.org/%40masonjames%2Fopen-security/latest",
    );
    expect(notice).toEqual({
      currentVersion: "0.1.0",
      latestVersion: "0.2.0",
      command: "npx @masonjames/open-security@latest",
    });
  });
  ...
  test("uses the configured npm registry", async () => {
    ...
    expect(requestedUrl).toBe(
      "https://registry.example.test/npm/%40masonjames%2Fopen-security/latest",
    );
  });

  test("prefers fork registry configuration and retains legacy fallback", async () => {
    const requestedUrls: string[] = [];
    const request = async (environment: NodeJS.ProcessEnv) => {
      await checkForUpdate({
        environment,
        currentVersion: "0.1.0",
        fetch: async (url) => {
    ...
    await request({
      OPEN_SECURITY_NPM_REGISTRY: "https://preferred.example.test",
      CODEX_SECURITY_NPM_REGISTRY: "https://legacy.example.test",
    ...
    await request({
      OPEN_SECURITY_NPM_REGISTRY: " ",
      CODEX_SECURITY_NPM_REGISTRY: "",
      npm_config_registry: "https://npm.example.test",
    });

    expect(requestedUrls).toEqual([
      "https://preferred.example.test/%40masonjames%2Fopen-security/latest",
      "https://legacy.example.test/%40masonjames%2Fopen-security/latest",
      "https://npm.example.test/%40masonjames%2Fopen-security/latest",
    ]);
  });

  test("recognizes npx and local or global npm, pnpm, Yarn, and Bun", () => {
    const installed = "/workspace/node_modules/pkg/dist/version.js";
    const packageName = "@masonjames/open-security@latest";
  ...
  test("suppresses registry checks in CI or when disabled", async () => {
    ...
    for (const environment of [
      { OPEN_SECURITY_NO_UPDATE_NOTICE: "1" },
  ...
  test("prints the update banner to interactive stderr without changing JSON", async () => {
    ...
    const notice = {
      currentVersion: "0.1.0",
      latestVersion: "0.2.0",
      command: "npm install -g @masonjames/open-security@latest",
    };
