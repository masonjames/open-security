...
describe("CLI", () => {
  ...
  test("documents user-facing environment and deep-scan configuration", async () => {
    ...
    expect(readme).toMatch(/\|\s*`OPEN_SECURITY_LOG_LEVEL`\s*\|\s*CLI-only\b/u);
    expect(readme).toMatch(
      /\|\s*`CODEX_SECURITY_LOG_LEVEL`\s*\|\s*Backward-compatible\b/u,
    );
  ...
  test.each([
    ...
    async (_name, provider, model, _providerConfig) => {
    ...
    for (const argv of [
      ...
      expect(
        await main(
          ...
          stdout.stream,
          capture().stream,
          dependencies({ onWorkbench: () => automaticComparison }),
        ),
      ).toBe(0);
  ...
  test("documents existing model and reasoning options in bulk-scan help", async () => {
    ...
    expect(help.text()).toContain("--model <string>");
    expect(help.text()).toContain("Model for each repository");
    expect(help.text()).toContain(
      "--provider <openai|openrouter|fireworks|amazon-bedrock>",
    );
    expect(help.text()).toContain("--reasoning-effort <string>");
  ...
  test.each([
    ...
    "keeps %s provider selection separate from Codex overrides",
    async (provider, model) => {
      let config: CodexSecurityConfig | undefined;
      expect(
        await main(
          ["scan", ".", "--provider", provider, "--model", model],
          capture().stream,
          capture().stream,
          dependencies({
            environment:
  ...
  test("keeps Amazon Bedrock provider selection separate from Codex overrides", async () => {
    let config: CodexSecurityConfig | undefined;
    expect(
      await main(
        [
  ...
  test("parses TOML override literals and rejects conflicts", () => {
    ...
    for (const provider of [
      ...
      expect(() =>
        parseCodexOverrides([], undefined, undefined, undefined, provider),
  ...
  test("enables verbose diagnostics through OPEN_SECURITY_LOG_LEVEL", async () => {
    const stdout = capture();
    const stderr = capture();
    ...
    expect(
      await main(
        ...
        stderr.stream,
        dependencies({
          environment: { OPEN_SECURITY_LOG_LEVEL: "  DeBuG  " },
        }),
      ),
    ).toBe(0);
    expect(JSON.parse(stdout.text())).toEqual(fakeResult().toJSON());
  ...
  test("prefers OPEN_SECURITY_LOG_LEVEL over legacy and shared log levels", async () => {
    const stdout = capture();
    const stderr = capture();
  ...
  test("redacts verbose output paths and observer diagnostics", async () => {
    ...
    expect(JSON.parse(stdout.text())).toEqual(fakeResult().toJSON());
    expect(stderr.text()).toContain(
      'open-security: debug: scan.output_archived archive_dir="/tmp/archive_[redacted]"',
    );
    expect(stderr.text()).toContain(
      'open-security: debug: scan.output_ready scan_dir="/tmp/scan_[redacted]"',
    );
    expect(stderr.text()).toContain(
  ...
  test("emits redacted scan warnings in verbose diagnostics", async () => {
    ...
    expect(JSON.parse(stdout.text())).toEqual(fakeResult().toJSON());
    expect(stderr.text()).toContain(
      'open-security: debug: scan.warning message="Repository HEAD changed during the scan: [redacted]"',
    );
    expect(stderr.text()).toContain(
      "open-security: warning: Repository HEAD changed during the scan: [redacted]",
    );
