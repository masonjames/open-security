...
const deepScanOwnershipProbe = [
  ...
  "existing_owner = case.get('existingOwner')",
  "connection.execute(\"INSERT INTO workspaces VALUES ('workspace', ?, 'before')\", (existing_owner,))",
  "connection.execute(\"INSERT INTO scans VALUES (?, 'workspace', 'deep', 'running', '{}', 'delivered', ?, ?, 'before')\", (scan_id, case['storedToken'], existing_owner))",
...
describe("deep scan workbench ownership", () => {
  ...
  test("joins a scan already owned by the requesting thread", () => {
    expect(
      runOwnershipProbe({
        storedToken: originalClaimToken,
        suppliedToken: originalClaimToken,
        existingOwner: "requesting-thread",
      }),
    ).toMatchObject({
      accepted: true,
      result: { startDisposition: "joined" },
